...
const MissedCalls = (props) => {
  ...
  const [missedCallLogs, setMissedCallLogs] = useState(null);
  ...
  useEffect(() => {
    sessionStorage.setItem("currentNavHeader", "Missed Calls");
    setNavHeader(sessionStorage.getItem("currentNavHeader"));
    setNavHeader("Missed Calls");
    setNavSubHeader("");
    setMissedCallLogs(JSON.parse(sessionStorage.getItem("callLogs")));
    // callLogsDb.find({}, function (err, docs) {
    //   let data = docs[0];
    //   console.log(data);
    //   console.log(docs);
    //   setMissedCallLogs(data);
    //   log.info("All Missed call logs fetched...");
    // });
  }, []);
  ...
  //     console.log(docs);
  //     setMissedCallLogs(data);
  //     log.info("All Missed Call logs Updated......");
  //   });
  // }, [db_log_update]);
                    ...
                    <CallsClicked
                      ...
                      dialcall={() => {
                        ...
                        log.info(
                          ...
                          info.number,
                          "through context right click in Missed calls"
                        );
                      }}
                    ...
                    >
                      <div key={i}>
                        <div className="row w-100 px-0  d-flex mb-3 calls py-2 justify-content-between">
                          ...
                          <div className="px-0 col-6 mx-3 ps-4 d-flex align-items-end ">
                            <span className="mx-1">
                                ...
                                <i
                                  ...
                                  onClick={() => {
                                    ...
                                    log.info(
                                      "Dilaer function called for",
                                      info.number,
                                      "through icon click in Missed calls"
                                    );
                                    setShowOngoing({
                                      flag: true,
                                      number: info.number,
    ...
    <div>
      ...
      Object.keys(JSON.parse(sessionStorage.getItem("callLogs"))).length > 0 ? (
        Object.entries(JSON.parse(sessionStorage.getItem("callLogs")))
          ...
          .reverse()
          .map(([keys, values]) => {
            if (keys !== "_id" && values.some((e) => e.call_type == 3))
      ...
      {/* {(missedCallLogs === "" ||
        missedCallLogs == undefined ||
        missedCallLogs == null ||
        missedCallLogs.length == 0) &&
