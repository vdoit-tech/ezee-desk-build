...
import incomingBeepTone from "../../assets/audio/incoming-beep.wav";
...
const OnGoingCall = ({ ses }) => {
  ...
  useEffect(() => {
    ...
    if (ses !== undefined && ses.outgoingInviteRequest !== undefined) {
  ...
  }, [ses !== undefined && ses._state]);

  useEffect(() => {
    ses !== undefined && console.log("session1", ses._state);

    if (
      ses !== undefined &&
      (ses._state === "Established" || ses._state === "Establishing")
    ) {
  ...
  return ses == undefined ? (
    ...
    >
      <audio
        ...
        controls="controls"
        src={incomingBeepTone}
        autoPlay={ses._state == "Initial"}
        loop="loop"
      ></audio>
